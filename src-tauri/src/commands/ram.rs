...
#[tauri::command]
pub async fn upload_to_ram(state: State<'_, Arc<AppState>>, path: String) -> Result<FileMeta, AppError> {
    ...
    state.ram.lock().unwrap().insert(meta.clone(), Arc::from(bytes.into_boxed_slice()))?;
    Ok(meta)
}
